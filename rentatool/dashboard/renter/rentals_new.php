...
// Get user rating with more reliable query
$userStmt = $pdo->prepare("
    SELECT u.*, 
           COALESCE((SELECT AVG(Rating) FROM Review 
                    WHERE ReviewedEntityID = u.UserID AND EntityType = 'User'), 0) as AvgRating
    FROM User u
...
<div class="container mx-auto my-8">
    <div class="flex justify-between items-center mb-6">
        ...
        <div class="text-sm text-gray-500">
            Member since: <?php echo date('M d, Y', strtotime($user['CreatedAt'])); ?>
            <?php if ($user['AvgRating'] > 0): ?>
            <div class="flex items-center mt-1">
                ...
                <?php for ($i = 1; $i <= 5; $i++): ?>
                    <svg class="w-4 h-4 <?php echo $i <= round($user['AvgRating']) ? 'text-yellow-400' : 'text-gray-300'; ?>" fill="currentColor" viewBox="0 0 20 20">
                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                    </svg>
                <?php endfor; ?>
                <span class="ml-1">(<?php echo number_format($user['AvgRating'], 1); ?>/5)</span>
            </div>
            <?php else: ?>
            <div class="text-sm text-gray-500 mt-1">No ratings yet</div>
            <?php endif; ?>
        </div>
    </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <?php foreach ($rentalRequests as $request): ?>
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900"><?php echo htmlspecialchars($request['OwnerFirstName'] . ' ' . $request['OwnerLastName']); ?></div>
                                <?php if ($request['OwnerRating'] > 0): ?>
                                <div class="flex items-center mt-1">
                                    <?php for ($i = 1; $i <= 5; $i++): ?>
                                    ...
                                    <?php endfor; ?>
                                    <span class="ml-1 text-xs text-gray-500">(<?php echo number_format($request['OwnerRating'], 1); ?>)</span>
                                </div>
                                <?php endif; ?>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
